...
//    private int laserSegmentsDrawn = 0;
    ...
    public void paintComponent(Graphics graphics) {
        ...
        for(int[] piece : whiteOnlyTiles) {
            graphics.setColor(new Color(180,180,180));
            graphics.fillRect(leftBorder + (piece[1] * (tileSize + borderSize)), topBorder + (piece[0] * (tileSize + borderSize)), tileSize, tileSize);
        }

        //draw red only tules
        for(int[] piece : redOnlyTiles) {
            graphics.setColor(new Color(255, 85, 80));
            graphics.fillRect(leftBorder + (piece[1] * (tileSize + borderSize)), topBorder + (piece[0] * (tileSize + borderSize)), tileSize, tileSize);
        }
    ...
    public void stackObelisk(int[] tileA, int[] tileB) {
        Obelisk obelisk = (Obelisk) board[tileA[0]][tileA[1]];
        if (obelisk.stacked)
    ...
    public ArrayList<Point> getPxPointFromTileList(ArrayList<int[]> tiles) {
        ...
        for(int i = 0; i < tiles.size(); i++) {
            ...
            Point point = board[set[0]][set[1]].getCenterPoint();
            
            if (set[0] == 0 && set[1] == 0) {
                point = new Point((int)point.getX(), 
                				  (int)point.getY() - ((tileSize / 2) - borderSize));
        ...
        if (laser.effect.equals("missed")) {
            ...
            if (lastDir == 0) {
                point = new Point((int)point.getX(),
                				  (int)point.getY() - tileSize);
            }
            if (lastDir == 90) {
                point = new Point((int)point.getX() + tileSize,
                				  (int)point.getY());
            }
            if (lastDir == 180) {
                point = new Point((int)point.getX(),
    ...
    public void removePiece(int row, int col) {
        GamePiece gamePiece = board[row][col];
        
        if (gamePiece instanceof Obelisk) {
            Obelisk obelisk = (Obelisk) gamePiece;
            
            if (obelisk.stacked) {
                obelisk.stacked = false;
            } else {
